...
def btnsend():
    ...
    if sendmsg == '':
        ...
        s2.send(sendmsg.encode('utf-8'))
        entry3.delete(0, END)
        msg = s2.recv(1024).decode('utf-8')
        scr1.insert("insert", msg)
        # client1.client1Insert()
...
def clientUI():
    ...
    Button(root, text="刷新", width=8, command=flush).grid(row=2, column=4, padx=8)
    scrolW = 58
    scrolH = 20
    global scr1
